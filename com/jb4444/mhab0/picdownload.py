...
class PicDownload(object):
    __path = os.path.join(sys.path[0], "pic")
    def __init__(self, text, delay=0):
        self.__delay = delay
        self.__path = self.__path+text
        if not (os.path.exists(self.__path) and os.path.isdir(self.__path)):
    ...
    def doDownload(self, urlorlist):
        if isinstance(urlorlist, list):
            for url in urlorlist:
                url = url.replace('.jsp', str(round(time.time()))+'.jsp')
                self.__download(url)
        else:
    ...
    def __download(self, url):
        try:
            ...
            if res.status_code == 200:
                ...
                name = str(round(time.time())) + url.split('/')[-1]
